    ...
    const mrpt::math::TTwist2D& velLocal)
{
  ...
  {  // Note: JLBC 23/Nov/2016: I have removed an estimation of velocity
     // from
    ...
    mrpt::Clock::time_point tim_query) const
{
  ...
  const TPose2D p =
      (CPose2D(m_last_loc) + (CPose2D(m_last_odo) - CPose2D(m_loc_odo_ref))).asTPose();
    ...
    const TPose2D& p, const mrpt::math::TTwist2D& velLocal, const double delta_time, TPose2D& new_p)
{
  ...
  {  // non-Ackermann-like vehicle:
     // extrapolate as a straight line:
