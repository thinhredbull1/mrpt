...
CMetricMapBuilderICP::CMetricMapBuilderICP() : ICP_options(m_min_verbosity_level)
{
...
CMetricMapBuilderICP::TConfigParams& CMetricMapBuilderICP::TConfigParams::operator=(
    ...
    const mrpt::config::CConfigFileBase& source, const std::string& section)
{
  ...
  verbosity_level =
      source.read_enum<mrpt::system::VerbosityLevel>(section, "verbosity_level", verbosity_level);
...
void CMetricMapBuilderICP::TConfigParams::dumpToTextStream(std::ostream& out) const
{
  ...
  out << mrpt::format("insertionLinDistance                    = %f m\n", insertionLinDistance);
  out << mrpt::format(
      "insertionAngDistance                    = %f deg\n", RAD2DEG(insertionAngDistance));
  out << mrpt::format("localizationLinDistance                 = %f m\n", localizationLinDistance);
  out << mrpt::format(
      "localizationAngDistance                 = %f deg\n", RAD2DEG(localizationAngDistance));
  out << mrpt::format(
      "verbosity_level                         = %s\n",
      mrpt::typemeta::TEnumType<mrpt::system::VerbosityLevel>::value2name(verbosity_level).c_str());
  ...
  else
  {
    ...
    if (!we_skip_ICP_pose_correction)
    {
      ...
      if (ICP_options.matchAgainstTheGrid &&
          ...
          metricMap.mapByClass<COccupancyGridMap2D>(0)->insertionOptions.useMapAltitude)
      {
        // Use grid altitude:
        if (IS_CLASS(*obs, CObservation2DRangeScan))
        {
          ...
          if (std::abs(
                  metricMap.mapByClass<COccupancyGridMap2D>(0)->insertionOptions.mapAltitude -
    ...
    const bool firstTimeForThisSensor =
        m_distSinceLastInsertion.find(obs->sensorLabel) == m_distSinceLastInsertion.end();
    bool update =
    ...
    if (options.alwaysInsertByClass.contains(obs->GetRuntimeClass())) update = true;
    ...
    if (options.enableMapUpdating && update)
    {
      ...
      // Create new entry:
      m_distSinceLastInsertion[obs->sensorLabel].last_update = currentKnownRobotPose.asTPose();
...
/*---------------------------------------------------------------
            initialize
  ---------------------------------------------------------------*/
void CMetricMapBuilderICP::initialize(const CSimpleMap& initialMap, const CPosePDF* x0)
{
  MRPT_START
  ...
  if (x0)
    m_lastPoseEst.processUpdateNewPoseLocalization(x0->getMeanVal().asTPose(), mrpt::Clock::now());
...
void CMetricMapBuilderICP::getCurrentMapPoints(std::vector<float>& x, std::vector<float>& y)
{
  auto lck = mrpt::lockHelper(critZoneChangingMap);
